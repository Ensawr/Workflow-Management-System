...
<body>
    ...
    <div class="card" id="card_container">
        ...
        <?php 
            ...
            $sendpath = "../Static/photos/$itemCode/ColorPhotos/$colorCode";
            $pathXcc = "../Static/photos/$itemCode/ColorPhotos/";
            $photos = scandir($path);
            if($photos<1){
                echo "<p style='font-size:30px; color:red'>Product Photos Not Uploaded !</p>";
            }
            else{
            array_shift($photos);
            array_shift($photos);
        ...
        ?>    
<form action="../../Controllers/product_edit_change.php?send=dataR" method="POST" enctype="multipart/form-data">
        ...
        <?php } ?>
            <input type="file" name="files[]" class="choose_file">
            <button type="submit" name="add" value="add" class="btn btn-outline-info add">Add</button>
        <?php } ?>
